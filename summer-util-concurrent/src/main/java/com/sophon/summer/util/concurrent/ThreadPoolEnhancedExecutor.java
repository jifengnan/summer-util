...
/**
 * All code is copied from the ThreadPoolExecutor, the only different is that
 * if there's no any idle thread, and fewer than maximumPoolSize (see {@link #getMaximumPoolSize}) threads are running,
 * the new execute method will create a new thread instead of to offer queue directly.
 * More details please see the comment "Core and maximum pool sizes" below.
 * <p>
 ...
 * maximumPoolSize threads running, and no any idle thread found,
 * a new thread will also be created.
 ...
 */
public class ThreadPoolEnhancedExecutor extends ThreadPoolExecutor {
    /**
    ...
    private static final int SHUTDOWN = 0; // 0 << COUNT_BITS still is 0
    ...
    private final AtomicInteger idleWorkerCount = new AtomicInteger();
     ...
     */
    public ThreadPoolEnhancedExecutor(int corePoolSize,
                                      int maximumPoolSize,
                                      long keepAliveTime,
                                      TimeUnit unit,
